...
const ROUTE_TITLES: Array<{ match: (path: string) => boolean; title: string }> =
  [
    ...
    { match: (path) => path === "/login", title: `Log in · ${APP_NAME}` },
    {
      ...
      title: `Invitation only · ${APP_NAME}`,
    },
    {
    ...
    {
      match: (path) => path === "/study-plan",
      title: `Study plan · ${APP_NAME}`,
    },
    { match: (path) => path === "/subjects", title: `Subjects · ${APP_NAME}` },
    {
      match: (path) => path.startsWith("/subjects/"),
      title: `Subject · ${APP_NAME}`,
    },
    {
      match: (path) => path === "/past-papers",
      title: `Past papers · ${APP_NAME}`,
    },
