...
```
*Output Verification:* In dry-run mode, parsing, normalization, validation, and result reporting still run, but the database upsert path is skipped entirely. No write transaction is opened and no database rows are changed.
